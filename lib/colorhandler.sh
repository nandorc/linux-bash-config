...
function color() {
    if [ -f ~/.basher/var/colors.ini ]; then
        declare text_style && declare font_color && declare bg_color && declare color_var && declare color_array && declare color_result
        text_style="0" && font_color="" && bg_color="" && color_result=""
        while [ -n "${1}" ]; do
