...
function checkElasticSearchService() {
    http_response_code=$(curl -s -w '%{http_code}' -o ~/.basher/src/bashcustomizer/src/elasticsearch/http_response -X GET 'http://localhost:9200/_cat/health?v&pretty')
    ...
    if [ "$1" = "string" ]; then
        ...
        if [ $int_status -eq 1 ]; then
            if [ -f ~/.basher/src/bashcustomizer/src/elasticsearch/http_response ]; then
                printMessage "Elasticsearch service response:" after
                cat ~/.basher/src/bashcustomizer/src/elasticsearch/http_response
            else
    ...
    rm -f ~/.basher/src/bashcustomizer/src/elasticsearch/http_response
...
function startElasticsearchService() {
    if [ $(checkElasticSearchService) -ne 1 ]; then
        ...
        if [ ! -f ~/.basher/src/bashcustomizer/src/elasticsearch/pid ]; then
            touch ~/.basher/src/bashcustomizer/src/elasticsearch/pid
        fi
        cat "$(getINIVar ~/.basher/src/bashcustomizer/etc/vars.ini elasticsearch_path)"/pid >~/.basher/src/bashcustomizer/src/elasticsearch/pid
    fi
...
function stopElasticSearchService() {
    if [ $(checkElasticSearchService) -eq 1 ]; then
        if [ ! -f ~/.basher/src/bashcustomizer/src/elasticsearch/pid ]; then
            ...
            service_pid=$(echo $java_services | grep "^$(cat ~/.basher/src/bashcustomizer/src/elasticsearch/pid)$")
            if [ -z "$service_pid" ] && [ ! -z "$java_services" ]; then
                ...
                pkill -F ~/.basher/src/bashcustomizer/src/elasticsearch/pid
                printMessage "Testing elasticsearch service..."
                while [ $(checkElasticSearchService) -eq 1 ]; do
                    sleep 2
                done
                rm -f ~/.basher/src/bashcustomizer/src/elasticsearch/pid
