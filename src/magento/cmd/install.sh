...
# Start services
commandBlockHeading "MAGENTO SERVICES SETUP" "${spaces}" 0
...
if [ ${must_force} -eq 1 ]; then
    ...
    echo "" && basher docker:install --no-spaces --compact --force
    echo "" && basher elasticsearch:install --no-spaces --compact --force
else
    ...
    echo "" && basher docker:install --no-spaces --compact
    echo "" && basher elasticsearch:install --no-spaces --compact
fi
[ ${spaces} -eq 1 ] && echo ""
